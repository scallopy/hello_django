...
def post_detail(request, pk, post_slug):
    post = get_object_or_404(Post.objects.order_by("-id"), pk=pk)
...
def test_cookie(request):
    if not request.COOKIES.get('color'):
        ...
        return HttpResponse("Your favorit color is {0}".format(request.COOKIES['color']))

def track_user(request):
    # store response in response variable
